          ...
          {projects.map((p) => (
            <Card key={p.id ?? p.name} className="group flex flex-col">
              ...
              <div className="mt-4 flex items-center gap-2">
                <Link href={`/studio/editor/${p.state?.docKind === 'youtube-trademark' ? 'youtube-trademark' : p.state?.docKind === 'unhcr-s2' ? 'unhcr-s2' : p.kind}?project=${encodeURIComponent(p.id ?? p.name)}`} className="flex-1">
