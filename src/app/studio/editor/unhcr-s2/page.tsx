...
function UnhcrEditorInner() {
  ...
  const [activeField, setActiveField] = useState<UnhcrS2OverlayKey>('unhcrNo');
  ...
  const editor = useDocumentEditor<UnhcrS2Snapshot>({
    kind: 'unhcr-s2',
    defaults: UNHCR_DEFAULTS,
    autosaveKey: `studio.autosave.unhcr-s2.${user?.id ?? 'anon'}`,
    loadExternal: () => externalCacheRef.current,
    normalize: (s) => normalizeUnhcrSnapshot(s as Partial<UnhcrS2Snapshot>),
  ...
  useEffect(() => {
    ...
    const persistCurrent = async (snapshot: UnhcrS2Snapshot) => {
      if (!user || stale()) return;
      const title = snapshot.unhcrNo.trim() ? `UNHCR S2 ${snapshot.unhcrNo.trim()}` : 'UNHCR ID Server 2';
      await saveUnhcrS2CurrentState({
        snapshot,
        title,
        subtitle: snapshot.name.trim() || undefined,
        createdBy: user.id,
      });
    };
    (async () => {
      ...
      if (action === 'apply-current' && current.record) {
        const hydrated = hydrateUnhcrS2CurrentSnapshot(current.record.doc);
        applyWorkspace(hydrated.snapshot, 'Saved editor state loaded');
        if (hydrated.upgraded) await persistCurrent(hydrated.snapshot);
        return;
      }
      if (action === 'load-error' || current.error) {
        applyWorkspace(unhcrS2InitialCurrentSnapshot(), 'Server 2 template loaded');
        toast.error(current.error ?? 'Could not load Server 2 editor state');
        return;
      }
      const initial = unhcrS2InitialCurrentSnapshot();
      applyWorkspace(initial, 'Server 2 template loaded');
  ...
  const setPhoto = useCallback(
    (patch: Partial<Pick<UnhcrS2Snapshot, 'photoX' | 'photoY' | 'photoW' | 'photoH' | 'photoDataUrl'>>) => {
      setSnapshot((prev) => ({
        ...prev,
  ...
  const persistToHistory = useCallback(async (): Promise<string | null> => {
    ...
    const title = snap.unhcrNo.trim() ? `UNHCR S2 ${snap.unhcrNo.trim()}` : 'UNHCR ID Server 2';
    const subtitle = snap.name.trim() || undefined;
    const recordId = unhcrS2HistoryRecordIdForSave(
      historyRecordId,
      snap.unhcrNo,
      newRecordId('unhcr-s2'),
    );
    ...
    const current = await saveUnhcrS2CurrentState({
      snapshot: snap,
      title,
      subtitle,
      createdBy: user.id,
    ...
    void logActivity({
      user_id: user.id,
      email: user.email,
      action: 'history.save.unhcr-s2',
      detail: `UNHCR S2 ${snap.unhcrNo || res.recordId}`,
    });
    return res.recordId;
  ...
  const saveAsProject = useCallback(async () => {
    ...
    const res = await saveProject({
      name: present.unhcrNo.trim() ? `UNHCR S2 ${present.unhcrNo.trim()}` : 'UNHCR ID Server 2',
      kind: 'unhcr',
      state: { ...normalizeUnhcrSnapshot(present), docKind: 'unhcr-s2' } as unknown as Record<string, unknown>,
      owner_id: user.id,
    });
    if (res.error) {
  ...
  const hitField = useCallback((x: number, y: number): UnhcrS2OverlayKey | null => {
    const snap = presentRef.current;
        ...
        >
          <div className="flex flex-col gap-3">
            <Select
              ...
              value={activeField}
              onChange={(e) => setActiveField(e.target.value as UnhcrS2OverlayKey)}
