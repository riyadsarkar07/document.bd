...
import { UNHCR_BARCODE_TEST_PAYLOAD, UNHCR_QR_TEST_PAYLOAD } from '../unhcrCodes';
...
export const UNHCR_BARCODE1_DEFAULT = {
  x: 70,
  y: 1028,
  w: UNHCR_BARCODE_DEFAULT.w,
  h: UNHCR_BARCODE_DEFAULT.h,
};
...
export function normalizeUnhcrSnapshot(s: Partial<UnhcrSnapshot>): UnhcrSnapshot {
  ...
  const photoDataUrl = typeof s.photoDataUrl === 'string' && s.photoDataUrl.startsWith('data:image/')
    ? s.photoDataUrl
    : null;
  const barcodePayload = typeof s.barcodePayload === 'string' && s.barcodePayload.trim()
    ? s.barcodePayload.trim()
    : UNHCR_BARCODE_TEST_PAYLOAD;
  const qrPayload = typeof s.qrPayload === 'string' && s.qrPayload.trim()
    ? s.qrPayload.trim()
