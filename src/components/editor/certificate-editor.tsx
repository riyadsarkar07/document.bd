...
import { TM_SECTIONS, TM_BACKGROUND, TM_SIGNATURE, TM_TEXT_FIELDS, TM_EXPORT_SCALE } from '@/lib/constants/tm';
...
export function CertificateEditor({ variant }: { variant: CertificateDocKind }) {
  return (
    <Suspense fallback={<div className="flex flex-1 items-center justify-center text-sm text-dimm">Loading editor…</div>}>
      <CertificateEditorInner variant={variant} />
    </Suspense>
  );
}

function CertificateEditorInner({ variant }: { variant: CertificateDocKind }) {
  const config = CERTIFICATE_VARIANTS[variant];
  ...
  const loadExternal = useCallback(() => {
    ...
    if (!projectId && !templateName && !recordNo) return null;
  ...
  const editor = useDocumentEditor<TMSnapshot>({
    ...
    autosaveKey: `${config.autosavePrefix}.${user?.id ?? 'anon'}`,
    loadExternal,
  });
  ...
  // Load external project/template/history state
  useEffect(() => {
    const projectId = searchParams.get('project');
    const templateName = searchParams.get('template');
    const recordNo = searchParams.get('record');
    ...
    (async () => {
      if (recordNo) {
        ...
        delete (snap as Record<string, unknown>).timestamp;
        delete (snap as Record<string, unknown>).createdBy;
        delete (snap as Record<string, unknown>).creatorEmail;
        delete (snap as Record<string, unknown>).publishStatus;
        delete (snap as Record<string, unknown>).publishedAt;
        delete (snap as Record<string, unknown>).publishCommitSha;
        delete (snap as Record<string, unknown>).publishError;
        externalCacheRef.current = { ...config.defaults, ...snap, docKind: config.kind };
  ...
  const reset = useCallback(() => {
    ...
    toast.info(config.resetMessage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const exportJpg = useCallback(async () => {
    ...
    link.download = `${config.filePrefix}-${presentRef.current.trademarkNo || 'export'}.jpg`;
    link.href = canvas.toDataURL('image/jpeg', 0.96);
    link.click();
    setStatus('Saving to Cloud Vault…');
    const res = await commitCertificate({ ...presentRef.current, docKind: config.kind }, user?.id, { docKind: config.kind });
    if (res.error) {
      setStatus('Sync error.');
      toast.error(`Vault error: ${res.error}`);
      return;
    }
    void logActivity({ user_id: user?.id, email: user?.email, action: `export.${config.activityPrefix}.jpg`, detail: `${config.kindLabel} ${presentRef.current.trademarkNo}` });
  ...
  const exportPdf = useCallback(async () => {
    ...
    pdf.save(`${config.filePrefix}-${presentRef.current.trademarkNo || 'export'}.pdf`);
    setStatus('Saving to Cloud Vault…');
    const res = await commitCertificate({ ...presentRef.current, docKind: config.kind }, user?.id, { docKind: config.kind });
    if (res.error) {
      setStatus('Sync error.');
    ...
    void logActivity({ user_id: user?.id, email: user?.email, action: `export.${config.activityPrefix}.pdf`, detail: `${config.kindLabel} ${presentRef.current.trademarkNo}` });
  ...
  const saveAsProject = useCallback(async () => {
    ...
    const res = await saveProject({
      name: config.projectName(present.trademarkNo),
      kind: 'tm',
      state: { ...present, docKind: config.kind } as unknown as Record<string, unknown>,
      owner_id: user.id,
    });
    if (res.error) {
    ...
    void logActivity({ user_id: user?.id, email: user?.email, action: 'project.save', detail: `${config.kindLabel} ${present.trademarkNo}` });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [present, user, toast]);
  ...
  const publishCertificate = useCallback(async () => {
    ...
    setBusy(true);
    setStatus('Saving to Cloud Vault…');
    const vault = await commitCertificate({ ...presentRef.current, docKind: config.kind }, user.id, { docKind: config.kind });
    if (vault.error) {
      setBusy(false);
    ...
    if (result.ok) {
      ...
      void logActivity({ user_id: user?.id, email: user?.email, action: 'publish.ui', detail: `${config.kindLabel} ${tmNo}` });
    } else {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
      ...
      <InspectorPanel
        ...
        subtitle={config.inspectorSubtitle}
