...
function main() {
  ...
  const auditSrc = readFileSync(join(ROOT, 'src/lib/workspace/audit.ts'), 'utf8');
  const historySrc = readFileSync(join(ROOT, 'src/app/studio/history/page.tsx'), 'utf8');
  const templatesSrc = readFileSync(join(ROOT, 'src/app/studio/templates/page.tsx'), 'utf8');
  const projectsSrc = readFileSync(join(ROOT, 'src/app/studio/projects/page.tsx'), 'utf8');
  const clientSrc = readFileSync(join(ROOT, 'src/lib/supabase/client.ts'), 'utf8');
  assert(clientSrc.includes('export const supabaseData'), 'workspace queries use a dedicated data client');
  assert(clientSrc.includes('accessToken: async () => ensureFreshAccessToken()'), 'data client refreshes the JWT instead of calling getSession');
  assert(!clientSrc.includes('accessToken: async () => getCachedAccessToken()'), 'data client never sends a raw cached JWT without an expiry check');
  assert(clientSrc.includes('export async function ensureFreshAccessToken'), 'expired access tokens are refreshed before PostgREST');
  assert(clientSrc.includes('refreshSession({ refresh_token: refreshToken })'), 'refresh uses the refresh token, not getSession/getUser');
  assert(clientSrc.includes('__studioRefreshInFlight'), 'token refresh is single-flight');
  assert(clientSrc.includes('invalidateStudioSession'), 'genuine refresh failures clear the authenticated session');
  assert(clientSrc.includes('export function settleWithTimeout'), 'workspace queries race against a hard timeout');
  assert(clientSrc.includes('WORKSPACE_QUERY_TIMEOUT_MS = 12000'), 'workspace query timeout is 12s');
  assert(!authSrc.includes('supabase.auth.getUser('), 'AuthProvider never calls getUser (GoTrue navigator.lock deadlock)');
  assert(authSrc.includes('supabase.auth.getSession()'), 'AuthProvider bootstraps from getSession');
  assert(authSrc.includes('getCachedSession()'), 'AuthProvider hydrates immediately from the cached session');
  assert(authSrc.includes('onAuthStateChange'), 'AuthProvider still listens for session changes');
  assert(authSrc.includes('setTimeout(() => {'), 'onAuthStateChange releases the auth lock before profile/workspace queries');
  assert(authSrc.includes("event === 'SIGNED_OUT'") && authSrc.includes('isRefreshingAccessToken()'), 'SIGNED_OUT during refresh does not wipe a recovering session');
  assert(authSrc.includes("apply(null, 'sign-out')"), 'explicit sign-out still clears authenticated state');
  assert(authSrc.includes('studio:session-invalid'), 'AuthProvider clears UI state when refresh proves the session is invalid');
  assert(authSrc.includes('.catch(() => {') && authSrc.includes('setLoading(false)'), 'AuthProvider still clears loading if getSession rejects');
  assert(authSrc.includes('bootstrapTimer') && authSrc.includes('setLoading(false)'), 'AuthProvider still clears loading if getSession never returns');
  assert(vault.includes('settleWithTimeout') && vault.includes('supabaseData'), 'vault list/load uses the data client with a timeout');
  assert(storeSrc.includes('settleWithTimeout') && storeSrc.includes('supabaseData'), 'templates/projects use the data client with a timeout');
  assert(storeSrc.includes('getCachedSession()'), 'workspace store scopes cache from the in-memory session');
  ...
  assert(auditSrc.includes('getCachedSession()'), 'audit trail reads the cached session');
  ...
  assert(vault.includes("error: err instanceof Error ? err.message : 'Could not load vault records.'"), 'vault list/load settles with a real error instead of hanging');
  assert(s2Editor.includes('getUnhcrS2CurrentState()') && !s2Editor.includes('listVaultRecords'), 'Server 2 still loads its own current state without History vault listing');
