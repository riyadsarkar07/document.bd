...
function main() {
  ...
  assert(storeSrc.includes('supabase.auth.getSession()'), 'workspace store scopes cache from getSession');
  assert(!storeSrc.includes('supabase.auth.getUser('), 'workspace store does not call getUser');
  assert(auditSrc.includes('supabase.auth.getSession()'), 'audit trail reads the local session');
  ...
  assert(dashboard.includes('} finally {') && dashboard.includes('setLoading(false)'), 'Dashboard always clears workspace loading');
