...
function main() {
  ...
  assert(clientSrc.includes('accessToken: async () => getCachedAccessToken()'), 'data client reads the cached JWT instead of getSession');
  ...
  assert(authSrc.includes('setTimeout(() => apply(session), 0)'), 'onAuthStateChange releases the auth lock before profile/workspace queries');
